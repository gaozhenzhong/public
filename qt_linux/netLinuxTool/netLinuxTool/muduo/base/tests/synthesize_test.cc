...
//extern muduo::AtomicInt32 muduo::Thread::numCreated_;


void mysleep(int seconds)
{
  timespec t = { seconds, 0 };
  nanosleep(&t, NULL);//这个函数功能是暂停某个线程直到你规定的时间后恢复，参数req就是你要暂停的时间，其中req->tv_sec是以秒为单位，而tv_nsec以纳秒为单位（10的-9次方秒）
}
...
int main()
{
  printf("pid=%d, tid=%d\n", ::getpid(), muduo::CurrentThread::tid());//getpid取得进程识别码
